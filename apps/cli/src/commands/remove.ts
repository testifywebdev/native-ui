...
import { fetchRegistryEntries, logError, logSuccess, logWarn } from '../utils.js';
...
export async function removeCommand(componentArgs: string[]) {
  ...
  if (componentArgs.length > 0) {
    ...
    const notInstalled = selectedKeys.filter(
      (k) => !installedKeys.includes(k)
    );
    ...
    const selected = await multiselect({
      ...
      options: installed.map((key) => ({
        value: key,
        label: pc.bold(installedEntries.get(key)?.title ?? key) + pc.dim(` — ${installedEntries.get(key)?.description ?? ''}`),
      })),
      required: true,
    });
  ...
  for (const key of selectedKeys) {
    ...
    const filePath = path.resolve(
      process.cwd(),
      comp?.files?.[0]?.target ?? comp?.files?.[0]?.path ?? key
    );

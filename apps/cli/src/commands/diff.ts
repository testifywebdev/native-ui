...
export async function diffCommand(componentArg?: string) {
  ...
  if (componentArg) {
    ...
    if (!(COMPONENTS as readonly string[]).includes(key)) {
      logError(`Unknown component: ${componentArg}. Run \`native-ui list\` to see available components.`);
      process.exit(1);
    }
    if (!config.components.includes(key)) {
      logError(`${componentArg} is not installed. Run \`native-ui add ${key}\` first.`);
      process.exit(1);
    }
    keysToCheck = [key];
  } else {
  ...
  for (const key of keysToCheck) {
    ...
    const localPath = path.resolve(process.cwd(), file?.target ?? file?.path ?? key);
    ...
    if (typeof remoteContent !== 'string') {
      results.push({ key, status: 'error', err: 'Registry returned no file content.' });
      continue;
    }
    ...
    results.push(
      isUpToDate(localContent, remoteContent)
        ? { key, status: 'upToDate' }
        : { key, status: 'changed', diff: lineDiff(localContent, remoteContent) }
    );
  }
  ...
  for (const r of results) {
    ...
    if (r.status === 'missing')  logWarn(`${name} — local file missing. Re-add: native-ui add ${r.key}`);
    if (r.status === 'error')    logError(`${name} — ${(r as { key: string; status: 'error'; err: string }).err}`);
  }

  const changed = results.filter(
    (r): r is { key: string; status: 'changed'; diff: string } => r.status === 'changed'
  );
  ...
  if (changed.length === 0) {
    ...
    const choice = await select({
      ...
      options: [
        { value: 'update-all', label: `Update all ${changed.length} component(s)` },

...
export async function initCommand(opts: { yes?: boolean; force?: boolean }) {
  ...
  if (configExists()) {
    try { existingComponents = readConfig().components; } catch { /* fresh install */ }
  }

  let config: NativeConfig = { ...DEFAULT_CONFIG, components: existingComponents };
...
const ok = installPackages(missingBaseDeps, config.expoRunner, projectRoot);
  ...
  outro(
    pc.cyan('Done! Run ') + pc.bold('native-ui add') + pc.cyan(' to add components.')
  );
}